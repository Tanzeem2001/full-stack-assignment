...
router.get('/', async (req, res) => {
  let totalRecords; // Declare totalRecords outside the try block
  ...
  try {
    ...
    console.error(error);
    const errorMessage = process.env.NODE_ENV === 'production' ? 'Internal Server Error' : error.message;
    res.status(500).json({ message: errorMessage, totalRecords }); // Send totalRecords in case of an error
  }
});
